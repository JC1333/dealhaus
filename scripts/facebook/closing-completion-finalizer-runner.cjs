...
async function runClosingStartRunner() {
  console.log("\nDEALHAUS COMPLETION FINALIZER RUNNER");
  console.log("-----------------------------");
  console.log(
    "Finding transactions confirmed complete by both buyer and seller."
  );
  ...
  const {
    ...
    .select(
      "id,inventory_item_id,item_title,buyer_name,sale_price,meetup_status,transaction_status,buyer_confirmed,seller_confirmed,negotiation_task_id,buyer_outreach_task_id,seller_lead_id"
    )
  ...
  if (!transactions?.length) {
    ...
    console.log(
      "\nCOMPLETION FINALIZER RUNNER COMPLETE"
    );
  ...
  const senderPath = path.join(
    ...
    "scripts",
    "facebook",
    "closing-completion-finalizer.cjs"
  );
