...
(async () => {
  ...
  if (tx.meetup_status !== "seller_preference_received") {
    console.log(
  ...
  const sellerReplyMatch = String(tx.notes || "").match(
  ...
  const sellerConfirmedDetails = sellerReplyMatch[1]
  ...
  const message =
    ...
    `They said: "${sellerConfirmedDetails}" ` +
    `Please let me know if that works for you and I'll keep everything coordinated from here.`;
  ...
  if (existingBody.includes(message)) {
    ...
    const {
      ...
      .update({
        ...
        notes: `${String(tx.notes || "").trim()}${String(tx.notes || "").trim() ? "\n\n" : ""}Seller-confirmed logistics relayed to ${buyerName}: "${sellerConfirmedDetails}"`,
      })
      .eq("id", tx.id)
      .select(
