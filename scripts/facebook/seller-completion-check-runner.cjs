...
async function runClosingStartRunner() {
  console.log("\nDEALHAUS SELLER COMPLETION CHECK RUNNER");
  console.log("-----------------------------");
  console.log(
    "Finding transactions ready for seller completion confirmation."
  );
  ...
  if (!transactions?.length) {
    ...
    console.log(
      "\nSELLER COMPLETION CHECK RUNNER COMPLETE"
    );
  ...
  const senderPath = path.join(
    ...
    "scripts",
    "facebook",
    "seller-completion-check-email.cjs"
  );
