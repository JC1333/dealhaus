...
(async () => {
   ...
   *
   * We DO NOT infer pickup, delivery, assembly,
   * location, fees, scheduling, or seller details.
   *
   * The next buyer reply can be processed against
   * the actual listing facts before seller contact.
   */
  const message =
    `Great, ${buyerName} — the $${acceptedPrice.toFixed(2)} ` +
    `purchase price is confirmed. ` +
    `For the next step, please let me know how you'd like ` +
    `to receive the item and any timing details that work for you. ` +
    `I'll use the listing terms to help coordinate the next steps.`;
